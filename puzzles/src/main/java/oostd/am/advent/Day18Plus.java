    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < input.size(); i++) {
            for (int j = 0; j < input.size(); j++) {
                if(i == j) continue;
                Pair left = (Pair) parse(input.get(i));
                Pair right = (Pair) parse(input.get(j));
                long mag = left.add(right).magnitude();
                if(mag > magnitude) {
                    magnitude = mag;
                    imax = i;
                    jmax = j;
...
//        Pair parse = (Pair)parse("[[2,[[7,7],7]],[[5,8],[[9,3],[0,2]]]]");
//        Pair parse1 = (Pair)parse("[[[0,[5,8]],[[1,7],[9,6]]],[[4,[1,2]],[[1,4],2]]]");
//
//        Pair add = parse.add(parse1);
