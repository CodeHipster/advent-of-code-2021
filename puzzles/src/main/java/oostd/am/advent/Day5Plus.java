    ...
    static class Line {
        ...
        List<Point> coveredPoints() {
            ...
            if (direction == Direction.HORIZONTAL) {
                ...
                int y = start.y;
                int x = start.x;
                for (;x <= end.x; x++, y+=yStep) {
                    coveredPoints.add(new Point(x, y));
                }
            }else {
                // step over y, assuming line is perfect vertical
                //TODO: implement proper y step for vertical lines, not required now as they are always exactly diagonal and thus handled by horizontal direction
                int x = start.x;
